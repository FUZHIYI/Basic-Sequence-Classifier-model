...
class SequenceClassificationModel(object):
	...
	def __init__(self, max_seqlen, n_class, vocab_size=None, embed_size=None, embed_matrix=None, embed_trainable=True, opt_type="adam", lr=1e-3):
			...
			with tf.device("/cpu:0"):
				self.embed_matrix = tf.concat(
					values=(tf.get_variable(
								initializer=tf.reshape(tf.convert_to_tensor(value=embed_matrix[0], dtype=tf.float32), shape=[1, self.embed_size]), 
							...
							tf.get_variable(
								initializer=tf.reshape(tf.convert_to_tensor(value=embed_matrix[-1], dtype=tf.float32), shape=[1, self.embed_size]), 
...
class UniRNNSequenceClassifier(SequenceClassificationModel):
	...
	def __build_core_graph(self):
		...
		with tf.variable_scope("rnn", reuse=tf.AUTO_REUSE):
			rnn_inputs = tf.nn.embedding_lookup(self.embed_matrix, self.X)
		...
		return logits, (rnn_outputs, final_state)
...
class HierarchicalAttentionClassifier(SequenceClassificationModel):
	...
	def build_graph(self):
		...
		self.logits, self.alpha_words, self.alpha_sents = self.__build_core_graph()
	...
	def __build_core_graph(self):
		...
		# word level
		rnn_inputs = tf.reshape(tf.nn.embedding_lookup(self.embed_matrix, self.X), 
								shape=[-1, self.max_seqlen, self.embed_size])
		with tf.variable_scope("word_level", reuse=tf.AUTO_REUSE):
			cell_fw = get_rnn_cell(self.cell_type, self.hidden_size)
			cell_bw = get_rnn_cell(self.cell_type, self.hidden_size)
			rnn_outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=cell_fw,
															 cell_bw=cell_bw,
															 inputs=rnn_inputs,
															 sequence_length=tf.reshape(self.L, shape=[-1]),
															 dtype=tf.float32)
			attn_inputs = tf.concat(rnn_outputs, 2)
			attn_outputs, alpha_words = intra_attention(attn_inputs, self.atten_size)

		# sent level
		rnn_inputs = tf.reshape(attn_outputs, [-1, self.max_textlen, 2*self.hidden_size])
		with tf.variable_scope("sent_level", reuse=tf.AUTO_REUSE):
			cell_fw = get_rnn_cell(self.cell_type, self.hidden_size)
			cell_bw = get_rnn_cell(self.cell_type, self.hidden_size)
			rnn_outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=cell_fw,
															 cell_bw=cell_bw,
															 inputs=rnn_inputs,
															 sequence_length=tf.reduce_sum(tf.cast(self.L>0, tf.int64), axis=1),
															 dtype=tf.float32)
			attn_inputs = tf.concat(rnn_outputs, 2)
			attn_outputs, alpha_sents = intra_attention(attn_inputs, self.atten_size)
		...
		return logits, alpha_words, alpha_sents
